...
export default function FilmDetails({ params }: any) {
  ...
  // Fix: Explicitly type 'film' so TypeScript knows it has properties like 'trailer_url'
  ...
  const bgVideoRef = useRef<any>(null); // Fix: Typed as any to prevent strict null errors
  ...
  // HLS Quality State
  const [qualities, setQualities] = useState<any[]>([]); // Fix: Typed array
  const [currentQuality, setCurrentQuality] = useState(-1); // -1 is Auto

  // Refs
  const videoRef = useRef<any>(null); // Fix: Typed refs
  ...
  useEffect(() => {
    if (showPlayer && film?.movie_url && videoRef.current) {
      if (Hls.isSupported() && film.movie_url.includes('.m3u8')) {
        ...
        // Fix: Typed event and data
        hls.on(Hls.Events.MANIFEST_PARSED, (event: any, data: any) => {
          setQualities(data.levels);
          videoRef.current.play().catch((e: any) => console.log("Auto-play prevented", e));
          setIsPlaying(true);
        });
  ...
  // Fix: Typed levelIndex parameter
        ...
        {film.cast_crew && film.cast_crew.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {film.cast_crew.map((member: any, idx: number) => (
